        ...
        if recursive and os.path.isdir(file):
            for dir_path, dir_names, file_names in os.walk(file):
                ...
                for pattern in exclude:
                    # os.walk() supports trimming down the dir_names list
                    # by modifying it in-place,
                    # to avoid unnecessary directory listings.
                    dir_names[:] = [
                        x for x in dir_names
                        if
                        not fnmatch.fnmatch(os.path.join(dir_path, x), pattern)
                    ]
                    file_paths = [
                        x for x in file_paths if not fnmatch.fnmatch(x, pattern)
                    ]
                for f in file_paths:
